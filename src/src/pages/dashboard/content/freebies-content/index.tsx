        ...
        <div className='w-full h-full bg-[#f9fbfc]'>
            <ModalView isVisible={openView} handleClose={handleCloseView} />
                ...
                <div className='mt-12 flex flex-col w-full min-h-[70dvh] text-[#555758]'>
                        ...
                        <div className='flex justify-end'>
                            <div className="relative h-10 w-48 min-w-[100px]">
                                ...
                                >
                                    ...
                                    <option value="QUANTITY_LOW_TO_HIGH">Price Low to High</option>
                                </select>
                                <label
                    ...
                    <div className='mt-8 bg-white w-full h-full max-h-[60dvh] min-h-[60dvh] border justify-center items-start flex overflow-auto p-24'>
                        <div className='w-full h-full grid grid-cols-3 gap-12'>
                            {freebiesData?.freebiesData ? (
                                freebiesData?.freebiesData.map((item, index) => (
                                    <div key={index} className='text-black text-2xl flex flex-col justify-center items-center border p-12'>
                                        <span>{item.freebiesName}</span>
                                        {item.freebiesImg && (
                                            <img src={`data:image/jpeg;base64,${item.freebiesImg}`} alt={item.freebiesName} />
                                        )}
                                    </div>
                                ))
                            ) : (
                                <div className="col-span-4 text-center">No data available</div>
                            )}
                        </div>
                    </div>
                </div>
