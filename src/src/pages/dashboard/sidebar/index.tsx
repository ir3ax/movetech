    ...
    <nav className='w-full h-full flex'>
      <div className='flex flex-col items-center w-full'>
        <div className='mt-16 mb-16 flex flex-col gap-2 justify-center items-center w-full'>
          <div className='w-24 h-24 rounded-full bg-[#C2C6FD]'></div>
          <span className='text-md font-bold text-[#5962FF]'>Mark Fernando</span>
        </div>
        <div className='w-full ml-8'>
          {sidebarData.map((item, index) => (
            <button
              ...
              key={index}
              className={`w-full flex justify-start items-start gap-4 p-4 text-${contentAtomValue === item.name ? '[#5962FF]' : '[#c9c7c7]'} hover:text-[#5962FF] active:text-[#5962FF] focus:text-[#5962FF]`}
            >
              {item.icons}
              <span>{item.name}</span>
            </button>
